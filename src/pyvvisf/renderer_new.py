...
try:
    ...
    # Define constants if OpenGL/GLFW is not available
    GL_TRIANGLE_STRIP = 5
...
class ISFRenderer:
    ...
    def render(self, width: int, height: int, inputs: Optional[Dict[str, Any]] = None, 
        ...
        validated_inputs = self.input_manager.get_merged_inputs(inputs, metadata)
        
        # Check for multi-pass rendering
        passes = getattr(metadata, 'passes', None) if metadata else None
        if passes and len(passes) > 1:
            return self._render_multipass(width, height, validated_inputs, metadata, time_offset)
        else:
            return self._render_singlepass(width, height, validated_inputs, metadata, time_offset)
