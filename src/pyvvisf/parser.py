...
from .types import ISFFloat, ISFInt, ISFValue, coerce_to_isf_value
...
class ISFInput(BaseModel):
    ...
    min: Any | None = None  # PATCH: allow float or list for point2D
    max: Any | None = None  # PATCH: allow float or list for point2D
    ...
    def validate_default(cls, v, info):
        ...
        if v is not None:
            ...
            try:
                ...
                raise ValueError(f"Invalid default value for type {expected_type}: {e}") from e
        return v
        ...
        if v is not None:
            if type_value == "point2D":
                if not (
                    ...
                    raise ValueError(
                        f"min for point2D must be a list or tuple of two numbers, got {v}"
                    )
            else:
        ...
        if v is not None:
            if type_value == "point2D":
                if not (
                    ...
                    raise ValueError(
                        f"max for point2D must be a list or tuple of two numbers, got {v}"
                    )
            else:
                if not isinstance(v, (int, float)):
...
class ISFPass(BaseModel):
    ...
    float_buffer: bool | None = None  # Renamed from 'float' to avoid shadowing built-in
    width: int | None = None
    height: int | None = None
    scaleX: float | None = None
    scaleY: float | None = None
    scaleToFit: bool | None = None
    scaleToFill: bool | None = None
    scaleToFitX: bool | None = None
    scaleToFitY: bool | None = None
    scaleToFillX: bool | None = None
    scaleToFillY: bool | None = None
    scaleToFitAspectRatio: bool | None = None
    scaleToFillAspectRatio: bool | None = None
...
class ISFMetadata(BaseModel):
    ...
    def validate_passes(cls, v, info):
        ...
        allowed_fields = {
            ...
            "scaleToFillX",
            "scaleToFillY",
            "scaleToFitAspectRatio",
            "scaleToFillAspectRatio",
            "filter",
        }
        target_names = set()
        for i, p in enumerate(v):
            if not isinstance(p, ISFPass):
                raise ValueError(
                    f"Each pass must be an ISFPass object (got {type(p)} at index {i})."
                )
            # Runtime check: get all fields present in the ISFPass instance
            for field in p.__dict__:
                if field not in allowed_fields:
                    raise ValueError(
                        f"Invalid field '{field}' in PASSES[{i}]. Allowed fields: {sorted(allowed_fields)}"
                    )
            # TARGET validation
            ...
            for dim in ("width", "height"):
                ...
                if val is not None:
                    if isinstance(val, (int, float)):
                        if val <= 0:
                            raise ValueError(
                                f"PASSES[{i}]: {dim.upper()} must be positive if specified."
                            )
                    elif isinstance(val, str):
                        # Accept ISF expressions like "$WIDTH/2.0"
                        if not val.startswith("$"):
                            raise ValueError(
                                f"PASSES[{i}]: {dim.upper()} string must start with '$' for ISF expressions."
                            )
                    else:
                        raise ValueError(
                            f"PASSES[{i}]: {dim.upper()} must be a number or ISF expression string."
                        )
            ...
            for bfield in ("persistent", "float_buffer"):
                ...
                if bval is not None and not isinstance(bval, bool):
                    raise ValueError(
                        f"PASSES[{i}]: {bfield.upper()} must be a boolean if specified."
                    )
            # FILTER validation
...
class ISFParser:
    ...
    def parse_content(self, content: str) -> tuple[str, ISFMetadata]:
        ...
        if not json_blocks:
            ...
            raise ISFParseError(
                "No ISF JSON metadata block found in shader content.", json_block=""
            )
        ...
        try:
            ...
            raise ShaderCompilationError(
                f"Failed to compile shader due to invalid ISF metadata: {e}"
            ) from e
            ...
            if "INPUTS" in metadata_dict:
                ...
                for input_data in metadata_dict["INPUTS"]:
                    if isinstance(input_data, dict):
                        ...
                        input_fields = {
                            k: v
                            for k, v in converted_input.items()
                            if k in ISFInput.__annotations__
                        }
    ...
    def _find_json_line(self, content: str, json_block: str) -> int | None:
    ...
    def validate_inputs(
        self, metadata: ISFMetadata, input_values: dict[str, Any]
    ) -> dict[str, ISFValue]:
