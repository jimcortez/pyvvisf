...
class ISFColor(ISFValue):
    ...
    @classmethod
    def from_tuple(cls, color_tuple: tuple[float, ...] | list[float]) -> "ISFColor":
...
class ISFPoint2D(ISFValue):
    ...
    @classmethod
    def from_tuple(cls, point_tuple: tuple[float, float] | list[float]) -> "ISFPoint2D":
        """Create from tuple or list."""
...
def coerce_to_isf_value(value: Any, expected_type: str = "auto") -> ISFValue:
    ...
    if expected_type == "auto":
        if isinstance(value, (tuple, list)):
            if len(value) == 2:
                ...
                raise ValueError(
                    f"Cannot auto-coerce {len(value)}-element sequence (got {value!r}). Primitives like int, float, bool, list, or tuple are allowed and will be coerced to the appropriate ISF type if possible."
                )
            ...
            raise ValueError(
                f"Cannot auto-coerce value of type {type(value)} (got {value!r}). Primitives like int, float, bool, list, or tuple are allowed and will be coerced to the appropriate ISF type if possible."
            )
    ...
    # Explicit type coercion
        ...
        if isinstance(value, (tuple, list)):
            ...
            raise ValueError(
                f"Cannot coerce value of type {type(value)} (got {value!r}) to color. Primitives like list or tuple of 3 or 4 floats are allowed."
            )
        ...
        if isinstance(value, (tuple, list)):
            ...
            raise ValueError(
                f"Cannot coerce value of type {type(value)} (got {value!r}) to point2D. Primitives like list or tuple of 2 floats are allowed."
            )
