...
from pyvvisf.errors import ISFParseError
from pyvvisf.parser import ISFInput, ISFMetadata, ISFParser
from pyvvisf.types import ISFBool, ISFColor, ISFFloat, ISFInt, ISFPoint2D


# 1. Standard Uniforms
@pytest.mark.parametrize(
    "uniform_name, glsl_type",
    [
        ("RENDERSIZE", "vec2"),
        ("TIME", "float"),
        ("TIMEDELTA", "float"),
...
# 2. Input Handling
@pytest.mark.parametrize(
    "input_type, value, expected_type",
    [
        ("bool", True, ISFBool),
        ("long", 42, ISFInt),
        ("float", 3.14, ISFFloat),
