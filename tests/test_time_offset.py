...
def test_time_offset_default():
    ...
    with pyvvisf.ISFRenderer(TEST_SHADER) as renderer:
        ...
        # Should be identical
        assert np.array_equal(arr_explicit, arr_default), (
            "Default and explicit time_offset=0.0 should be identical"
        )
...
def test_time_offset_buffer():
    ...
    with pyvvisf.ISFRenderer(TEST_SHADER) as renderer:
        ...
        assert not np.array_equal(arr_0s, arr_4s), (
            "Buffers at different time offsets should be different"
        )
...
def test_time_offset_negative():
    ...
    with pyvvisf.ISFRenderer(TEST_SHADER) as renderer:
        ...
        assert not np.array_equal(arr_neg, arr_zero), (
            "Negative time offset should produce different result"
        )
